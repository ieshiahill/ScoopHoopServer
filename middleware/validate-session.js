...
module.exports = function(req, res, next) {
    ...
    console.log(sessionToken) //The token is printed to the console. This is purely for debugging purposes to verify that the token is being sent to the server. It should not be left in the final code, as it is a potential security vulnerability.
    if (!sessionToken) return res.status(403).send({ auth: false, message: "No token provided." }) //If no token is present, the 403 Forbidden error is returned as the response. We have several different error handling responses in this file, so assigning each a different error code or message is a big help in debugging.
        ...
        jwt.verify(sessionToken, process.env.JWT_SECRET, (err,decoded) => { //The verify method decodes the token with the provided secret, then sends a callback with two variables. If successful, decoded will contain the decoded payload; if not, decoded remains undefined. err is null by default.
            if(decoded){
